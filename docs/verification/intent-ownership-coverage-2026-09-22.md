...
Issue #11's done-means asks for a before/after run on a real build; that run is
[intent-ownership-before-after-2026-09-22.md](intent-ownership-before-after-2026-09-22.md).
...
|---|---|---|---|
| plan-drift out-of-plan-file (studious `scripts/plan-drift:97-103`) | a changed file no task path names | `out_of_intent_files`: files with a trace `revert` or `trust boundary` hold, pre-dedup, pre-apply — `scripts/report.py:369-375`, `reference/report.md:120-136` | partial — content-based, not name-based; see consumer decisions |
| plan-drift do-path-untouched (`scripts/plan-drift:105-111`) | a `Do:` path the commits never touched | reverse roll call, named path absent from `git diff --name-only` → `unmet claim` — `agents/intent-tracer.md:44-45`, `:84-87` | covered, when the `Do:` path is in the claims |
...
| Inspector contract-match lens (studious `skills/build/SKILL.md:417-419`) | a shipped contract that does not match its design section or `Do`/`Done means` | reverse roll call, contract that contradicts the claim or its quoted design section → `unmet claim` — `agents/intent-tracer.md:48-50`, a claim that one file matches another — `:53-54`; `implied by intent` for what must stay — `:79-80` | partial — judged once per build after downstream tasks built on it, not per task before them |
| product-reviewer, unspecced scope built (gauntlet `agents/product-reviewer.md:85-87`) | something built that the spec never called for | forward roll call → `revert` — `agents/intent-tracer.md:27-35`, `:77-78` | covered, against the intent exorcise is handed, not the full design doc |
...
| held-finding context for the PR body (exorcist `reference/findings.md`) | `spec conflict`, `behavior change`, trust-boundary holds gauntlet stops re-raising | tracer emits `spec conflict` and `behavior change` — `agents/intent-tracer.md:59-71`; every hold carries `claim` and `next` — `:93-97`; `held[]` contract — `reference/report.md:111-119`; Held text line quotes the claim — `commands/exorcise.md:176-191` | covered |
...
| code-auditor dead code paths (`:90-91`) | branches that can no longer execute | superseded paths, always-on flags — `agents/deletion-scout.md:16-28`; a branch the diff adds that no caller can reach — `agents/abstraction-hunter.md:66-68` | covered |
...
| code-auditor hygiene (`:106-107`, `:91-92`) | debug logging, commented-out code, unused variables, TODOs | partial overlap: file-norm slop — `agents/deletion-scout.md:31-45`; orphan cleanup — `agents/intent-tracer.md:20-21` | keep — not in #89's list |
...
- **Technicality gaming.** A claim met in letter only — a test that asserts nothing, a
  ...
  (`agents/intent-tracer.md:56-57`). studious#442 should keep the Inspector's
  technicality lens.
...
Run once, as a replay of one studious build rather than a live build per repo:
[intent-ownership-before-after-2026-09-22.md](intent-ownership-before-after-2026-09-22.md).
Verdict **no**: 10 of 23 before findings lost, one of them in a `covered` row. The three
defects exorcist should own now have rules (`agents/intent-tracer.md:53-54`, `:63-68`,
`agents/deletion-scout.md:24-26`), and the rest are named there as kept or as gaps. The
protocol below still stands for the live runs.

One real studious build, the same PLAN.md, in each of the 3 repos (exorcist,
