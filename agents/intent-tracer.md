...
- `hold: "behavior change"` — a hunk a claim reaches that alters behavior a test
  ...
  test's `path:line` and what it asserts. `leads.json` lists the tests that still
  reference a value the diff retires: open and read each one, never judge it from its
  grep line; a value with `dropped` above 0 has more — grep it for the rest. One whose assertion the diff breaks is this hold; one read and cleared is
  no finding.
